                                    ...
                                    <td class="px-1 py-5 text-sm w-2/8">
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->client_user->detail_user->photo)
                                                    <img class="object-cover w-full h-full rounded-full" src="{{ url(Storage::url($order->client_user->detail_user->photo)) }}" alt="user photo" loading="lazy" />
                                                @else
                                            ...
                                            <div>
                                                <p class="font-medium text-black">{{ $order->client_user->name ?? '' }}</p>
                                                <p class="text-sm text-gray-400">{{ $order->client_user->detail_user->contact_number }}</p>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            <div class="">
                                <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                                    <div class="m-auto text-center">
                                        ...
                                        <div class="flex flex-col items-center relative mt-0 md:mt-6">
                                            @if (isset($order->file))
                                            ...
                                            @endif
                                            <label for="file" class="px-4 py-2 mt-2 mb-2 text-left text-gray-700 rounded-xl bg-serv-hr cursor-pointer">
                                                Choose File
                                            </label>
                                            <input type="file" accept=".zip" name="file" id="file" {{ (isset($order->file) ? '' : 'required') }} hidden>
                                            @if ($errors->has('file'))
                                                <p class="text-red-500 text-sm mb-3">{{ $errors->first('file') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
