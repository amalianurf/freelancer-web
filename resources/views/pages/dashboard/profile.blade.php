                        ...
                        <form action="{{ route('member.profile.update', Auth::user()->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="">
                                <div class="px-4 py-5 sm:p-6">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            <div class="flex items-center mt-1">
                                                @if (auth()->user()->detail_user()->first()->photo)
                                                    <img src="{{ url(Storage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="photo profile" class="inline-block w-16 h-16 rounded-full">
                                                @else
                                                ...
                                                @endif
                                                <label for="choose-photo" class="px-3 py-2 ml-5 text-sm font-medium leading-4 text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                                    Choose File
                                                </label>
                                                <input type="file" accept="image/*" id="choose-photo" name="photo" hidden>
                                                <a href="{{ route('member.delete.photo.profile') }}" class="px-3 py-2 ml-5 text-sm font-medium leading-4 text-red-700 bg-transparent rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return comfirm('Are you sure want to delete your photo?')">
                                                    Delete
                                                </a>

                                                @if ($errors->has('photo'))
                                                    <p class="text-red-500 text-sm mb-3">{{ $errors->first('photo') }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="md:col-span-6 lg:col-span-3">
                                            ...
                                            @if ($errors->has('role'))
                                                <p class="text-red-500 text-sm mb-3">{{ $errors->first('role') }}</p>
                                            @endif
                                        </div>
                                        <div class="md:col-span-6 lg:col-span-3">
                                            <label for="email" class="block mb-3 font-medium text-gray-700 text-md">Email Address</label>
                                            <input placeholder="youremail@mail.com" type="email" name="email" id="email" autocomplete="email" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" value="{{ $user->email ?? '' }}" required>
                                        ...
                                        <div class="md:col-span-6 lg:col-span-3">
                                            ...
                                            @if ($errors->has('contact_number'))
                                                <p class="text-red-500 text-sm mb-3">{{ $errors->first('contact_number') }}</p>
                                            @endif
                                        </div>
                                        <div class="col-span-6">
                                            ...
                                            @if ($errors->has('biography'))
                                                <p class="text-red-500 text-sm mb-3">{{ $errors->first('biography') }}</p>
                                            @endif
                                        </div>
                                        <div class="col-span-6">
                                            ...
                                            @forelse ($experience_user as $key => $item)
                                                ...
                                                @if ($errors->has('experience['.$item->id.']'))
                                                    <p class="text-red-500 text-sm mb-3">{{ $errors->first('experience') }}</p>
                                                @endif
                                            @empty
                                ...
                                <div class="px-4 py-3 text-right sm:px-6">
                                    <a href="{{ route('member.dashboard.index') }}" class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300" onclick="return confirm('Are you sure want to cancel? Any change you make will not be saved.')">
                                        Cancel
                                    </a>
