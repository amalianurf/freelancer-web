        ...
        <div class="grid gap-5 md:grid-cols-12">
            <main class="p-4 lg:col-span-8 md:col-span-12">
                ...
                <div class="p-3 my-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                    <img :src="featured" alt="thumbnail" class="rounded-lg cursor-pointer object-cover w-full" style="height: 500px" data-lity>
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        <div class="flex mt-2 flex-nowrap">
                            @forelse ($thumbnail as $item)
                                <img :class="{'border-4 border-serv-button': active === {{ $item->id }}}" @click="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})" src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail" class="inline-block mr-2 rounded-lg cursor-pointer w-36">
                            @empty
                                {{-- empty --}}
                            @endforelse
                        </div>
                ...
                <div class="content">
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    <div class="flex items-center space-x-4">
                                        @if ($service->user->detail_user->photo)
                                            <img src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="user profile" class="w-20 h-20 rounded-full ">
                                        @else
                                            <svg class="w-20 h-20 rounded-full bg-gray-100 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="flex-grow p-4 -mt-8 leading-8 lg:mt-0">
                                        ...
                                        <div class="text-gray-400">{{ $service->user->detail_user->role ?? '' }}</div>
                                    </div>
                                </div>
                            ...
                            <div class="mt-4 mb-8 content-description">
                                <p>{{ $service->user->detail_user->biography ?? '' }}</p>
                            </div>
            ...
            <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-start px-4 pt-6">
                        @if ($service->user->detail_user->photo)
                            <img class="object-cover w-16 h-16 mr-4 rounded-full" src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="user profile">
                        @else
                            <svg class="object-cover w-16 h-16 mr-4 rounded-full bg-gray-100 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                        @endif
                        <div class="w-full">
                    ...
                    <div class="px-4 pt-4 pb-2 features-list">
                        <ul class="mb-4 text-sm list-check">
                            @forelse ($tagline as $item)
                                <li class="pl-10 my-4">{{ $item->tagline }}</li>
                            @empty
                                {{-- empty --}}
                            @endforelse
                        </ul>
                    ...
                    <div class="px-4 pb-4 booking">
                        <a href="{{ route('landing.booking', $service->id) }}" class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
                            Booking Now
                        </a>
                    </div>
                </div>
...
@push('after-script')
    <script>
        function gallery() {
            return {
                featured: '{{ url(Storage::url($service->thumbnail[0]->thumbnail)) }}',
                active: {{ $service->thumbnail[0]->id }},
