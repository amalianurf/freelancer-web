...
@section('content')
    <section class="w-auto flex min-h-[85vh] pt-6 pb-20 mx-auto lg:mx-12 px-4">
        <div class="m-auto text-center h-auto">
            <svg class="mb-6 w-full" width="490" height="350" viewBox="0 0 490 350" fill="none"
                ...
                <defs>
                    ...
                    @if ($order->client_user->detail_user->photo)
                        <image id="image0" width="500" height="500" xlink:href="{{ url(Storage::url($order->client_user->detail_user->photo)) }}"/>
                    @else
                        <image id="image0" width="500" height="500" 
                            href="data:image/svg+xml;charset=UTF-8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'><circle cx='12' cy='12' r='12' fill='%23f3f4f6'/><path d='M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z' fill='%23d1d5db'/></svg>"
                        />
                    @endif
                    <!-- Gambar kanan -->
                    @if ($order->freelancer_user->detail_user->photo)
                        <image id="image1" width="500" height="500" xlink:href="{{ url(Storage::url($order->freelancer_user->detail_user->photo)) }}"/>
                    @else
            ...
            <a href="{{ url('http://wa.me/'.$order->freelancer_user->detail_user->contact_number) }}" class="bg-serv-button text-white text-md font-medium py-4 w-80 my-2 rounded-2xl text-center inline-block">
                Continue to Chat
            </a>
            <br>
